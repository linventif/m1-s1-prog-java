...
public class Day_2022_03 {
    private static final String PATH_DATA = "src" + File.separator + "data" + File.separator;

    public static List<Map.Entry<Character, Integer>> listSharedItems = new ArrayList<>();
        ...
        try (Scanner scanner = new Scanner(new FileInputStream(PATH_DATA + fichier))) {
            ...
            while (scanner.hasNextLine()) {
                ...
                if (!ch.isEmpty()) {
                    ...
                    entryCount++;
                    System.out.print("[" + String.format("%03d", entryCount) + "] Size: " + ch.length() + " : ");

                    // split in two
                    String firstHalf = ch.substring(0, ch.length() / 2);
                    String secondHalf = ch.substring(ch.length() / 2);
                    System.out.print(firstHalf + " | " + secondHalf);

                    // find shared chars
                    List<Character> sharedChars = new ArrayList<>();
                    for (char c : firstHalf.toCharArray()) {
                        if (secondHalf.indexOf(c) != -1 && sharedChars.indexOf(c) == -1) {
                            sharedChars.add(c);
                            // increase count
                            itemsCount.put(c, itemsCount.getOrDefault(c, 0) + 1);
                            int priority = (c >= 'a' && c <= 'z') ? (c - 'a' + 1) : (c - 'A' + 27);
                            System.out.print("  Shared: " + c + " (priority " + priority + ")");
                            sum += priority;
                        }
                    }
                }
            ...
            System.err.println(fichier + " is missing ! " + PATH_DATA);
        }
    }
}
